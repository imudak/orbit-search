...
## 昼夜判定処理の修正（2025/3/14追加）

軌道の昼夜表示に関する問題を修正しました。

### 問題点

- 軌道の点線（破線）が夜間を示すはずなのに、昼間の範囲になっていた
- 昼夜の判定処理が正しく機能していなかった
- 特に東経地域（日本など）での昼夜の判定が逆になっていた

### 原因

`calculateSolarPosition`関数の時角計算に問題がありました：

```typescript
// 問題のあるコード
const utcHours = date.getUTCHours() + date.getUTCMinutes() / 60 + date.getUTCSeconds() / 3600;
const hourAngle = (utcHours - 12) * 15 + lng; // 経度の符号が逆
```

経度（lng）の扱い方が逆になっていたため、昼夜の判定が正しく行われていませんでした。太陽は東から西へ移動するため、東経の場合は時角が小さくなり、西経の場合は時角が大きくなるのが正しい計算方法です。

### 実装した改善策

`sunCalculations.ts`ファイル内の`calculateSolarPosition`関数の時角計算を修正しました：
...
- 軌道の昼夜表示が正しく機能するようになりました
- 昼間の範囲は実線、夜間の範囲は破線で正しく表示されるようになりました
- 特に東経地域（日本など）での昼夜の判定が正確になりました
